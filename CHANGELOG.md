...
- Fixed: 动态目标 `$discord` 在签名、脚本分析和同语言比较前解析为 Discord 客户端实际语言；Discord English 下的英语历史不再被当成独立目标语言任务。失败历史消息按消息 ID 与配置签名停留在重试账本，只有显式“重试”或真实编辑/配置变化才重新进入，阻断 `hist` 周期增长和数秒一次的整区重绘
