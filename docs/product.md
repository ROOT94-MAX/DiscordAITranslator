...
- One historical display transaction commits the entire batch, then targets only its mounted ordinary message rows through the Store dispatcher. Ordinary confirmation failures stay on the bounded targeted retry path and never widen into a whole-chat rebuild; purely virtualized rows render their stored state when they mount. Reply-preview hosts and explicit channel/lifecycle refreshes remain separately tracked render-boundary work.
