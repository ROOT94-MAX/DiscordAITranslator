...
- The dynamic `$discord` language means Discord's current client language at decision time, not a separate language ID. Same-language filtering, request identity, target validation, and provider dispatch use that resolved language consistently.
