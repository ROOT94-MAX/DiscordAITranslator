...
- If rendered and cached messages provide fewer eligible items than the configured limit, the initial loaded-message pass may perform at most one bounded background prefetch request for the missing quantity. The job may therefore remain below the configured maximum; it does not simulate scrolling, inject prefetched messages into the visible list, or automatically page again.
...
- Enabling automatic translation while reading history preserves the current reading neighbourhood and eye-line message. A recent manual-translation lock belongs only to that message's own transaction and does not redirect channel enablement. The current visible anchor supplies one Store projection pulse; immediate and settled viewport restoration remain guarded by the user's scroll-intent sequence, without remounting the Composer.
...
- One historical display transaction commits the entire batch, then targets only its mounted ordinary and reply-host rows through the Store dispatcher. Body and preview surfaces carry separate DOM revisions; confirmation failures stay on bounded targeted retry and never widen into a whole-chat rebuild. Purely virtualized rows render their stored state when they mount. Channel enablement and primary-engine changes use one anchored Store projection pulse; plugin start/stop and settings reinitialization remain separate lifecycle work.
