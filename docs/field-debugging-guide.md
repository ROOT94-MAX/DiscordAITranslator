...
3. Message deletion is now wired locally through direct Store dispatcher subscriptions with no global dispatch patch. Automated start/stop/rollback and cleanup coverage passes; one PTB single/bulk-delete event-shape observation remains before completion is claimed.
