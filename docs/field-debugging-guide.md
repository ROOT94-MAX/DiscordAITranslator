...
This is the canonical handoff for the display-unification slices and the 2026-08-18 through 2026-08-21 PTB field-debugging session. It starts at the first slice/recon decision, follows every reported symptom through the v0.3.41 Composer-isolation closure, and separates proven causes from hypotheses and remaining observations.
...
7. **Row repaint evidence:** read-only probes showed no usable exported BDFDB dispatcher, then found a Store dispatcher route. The first experiment proved merge safety but did not count target content renders. The reopened probe confirmed one message-list projection render with Composer/input/row/scroller identities preserved; exact target visibility still belongs to DOM revision confirmation. Ordinary bodies and reply hosts now own independent revisions and bounded targeted retry; neither widens into a whole-chat fallback.
8. **Jump to newest during active history viewing:** before Store targeting, a fallback remount could strand the viewport at bottom—a position an in-history upward gesture did not request. Current historical paint waits behind the active-scroll gate and does not remount the chat; the viewport owner still restores the captured reading line after translated-height changes when user intent is unchanged.
...
10. **Enable auto-translate while reading an old date:** a stale 4.5-second manual anchor was once reused during a lifecycle repaint, and a virtualized missing row caused restore to return without using the current raw offset. Channel enablement now pulses one visible Store projection under a fresh center anchor; immediate and settled restore reuse the single viewport owner and remain vetoed by newer user intent.
...
| --- | --- | --- | --- |
| Composer icon blinked whenever translations appeared | Whole-chat rebuild unmounted the composer | Ordinary messages and preview hosts use separate Store revisions; channel/provider refresh uses one anchored projection pulse; plugin lifecycle stays separate | Do not treat a faster rebuild as a row repaint or widen any message surface |
...
| Atomic rebuild made scrolling worse | Once handles were wired, synchronous blank/remount forced every transaction to rebuild | Retired implementation and adapter seam removed; translation surfaces never widen into that rebuild | Do not restore synchronous blank/remount |
...
| Scroll jumped to newest and stayed there | The retired fallback remount defaulted to bottom; gesture veto protected the wrong post-remount position | Store-targeted paint avoids remount; defer historical paint during gestures and restore the center anchor only under unchanged intent | Do not repaint over an active gesture or remove user-intent discrimination |
...
| Enabling auto translation in old history jumped to newest | Lifecycle repaint reused a stale manual anchor and lacked missing-row raw-offset fallback | One visible Store projection pulse under a fresh center anchor, followed by intent-vetoed settling | Manual anchors belong only to transactions containing that message ID |
