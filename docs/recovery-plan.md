...
- Capsule showed "0/30" with processed stuck at zero after manual translation activity (screenshot evidence in session). Likely the processed/displayPending accounting or manual work not counted into the batch - reproduce and record the trigger before fixing, together with the per-batch-vs-cumulative product decision in the backlog.
