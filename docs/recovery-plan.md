...
The first-cut PTB smoke exposed three connected configuration-transition gaps. A translation painted under the previous target language could lose its restore proof; old completed IDs and failed retries survived into the new capsule; and dynamic `$discord` was compared as a literal ID instead of Discord English, so English history entered work and matching failures were automatically re-collected on later renders. The local corrections restore immutable source, reset the affected channel session only on a real configuration change, resolve `$discord` before policy/signature comparisons, and park failed items until explicit retry or a new source/configuration. Repeat the English-target/old-Chinese-cache scroll scenario before merging this cut or selecting the next wiring owner.
