...
The first-cut PTB smoke exposed three connected configuration-transition gaps. A translation painted under the previous target language could lose its restore proof; old completed IDs and failed retries survived into the new capsule; and dynamic `$discord` was compared/dispatched as a literal special ID instead of Discord English, so English history entered work, same-language echoes became failures, and matching failures were automatically re-collected. The local corrections restore immutable source, reset the affected channel session only on a real configuration change, turn same-language echoes into terminal skips, and park failed items until explicit retry or a new source/configuration. Field testing then confirmed that choosing explicit Simplified Chinese translates loaded English history normally, so the moving `$discord` output target is now retired: both output selectors expose only concrete languages and old global/server/channel aliases migrate once to the client's current concrete locale. Repeat the explicit-target scroll scenario before merging this cut or selecting the next wiring owner.
