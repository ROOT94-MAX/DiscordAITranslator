...
The first-cut PTB smoke exposed two configuration-transition gaps. A translation painted under the previous target language could lose its restore proof and become the next captured source; old per-channel completed IDs and failed retries also survived into the new target-language capsule. The local corrections restore immutable source through both render paths, admit one replacement decision, and reset the affected channel's cumulative/seen/failure/queue/boundary session only when the effective configuration signature changes. Repeat the English-target/old-Chinese-cache scroll scenario before merging this cut or selecting the next wiring owner.
