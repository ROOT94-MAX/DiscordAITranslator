...
The tenth resumed cut moved Live Translation Queue runtime/channel predicates, display commits, historical handoff, channel-session cleanup, batch result policy, cache commits, and single-item fallback wiring into `live-translation-queue-wiring.js`. The queue still owns newest-first ordering, locks, same-channel burst limits, handoff reservations, request identity, and retry cadence; the wiring now supplies BDFDB-managed timeout/clear ports so the 900 ms retry cannot survive plugin stop/reload as a raw global timer. The legacy runtime keeps only its lazy singleton boundary and the ratchet dropped from 3,329 to 3,260 lines. All 1,309 tests passed; installed deterministic build `fe6085ad52351072` left the complete configuration byte-identical. The final live-priority, historical-handoff, and channel-isolation field gate remains before merge.
