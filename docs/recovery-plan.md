...
The fourth resumed cut moves Message Viewport document, managed timer, animation-frame, selected-channel, scroller-selector, CSS escaping, and scroll-idle callback wiring into `message-viewport-wiring.js`. The store's scroll intent, reading-line anchor, bottom rescue, manual lock, input focus, and 180/600 ms settle rules remain unchanged; the legacy runtime keeps only its lazy singleton boundary and the ratchet drops from 3,415 to 3,404 lines. Deterministic build parity and all 1,290 tests pass; installed build `abaa550cd78f3382` left the complete configuration byte-identical. PTB confirmed history position, active-scroll veto, bottom/input focus, and cross-channel isolation. Returning to channel A landed at the native client bottom position, but channel B was never moved by A's restore work; that host navigation behavior is accepted and the cut is ready for fast-forward merge.
