...
**Status: ORDINARY ROW FIELD ROUTE CONFIRMED; PREVIEW HOST CUT IMPLEMENTED, FIELD GATE OPEN.** Ordinary live, cached, historical, manual, and ordinary restore transactions remain on the Store-targeted route. Reply-preview commit, clear, restore, and delete now target their host rows with a separate surface revision and at most three 300 ms attempts. Virtualized rows paint on mount. The implementation preserves the 500 ms historical quiet window, one historical display transaction, live priority, channel isolation, and the single viewport owner.

Current-client evidence for probe `1eb82dee729eaf05` shows one synthetic `MESSAGE_UPDATE` caused one message-list projection render while preserving the Composer, active input element, target row, scroller, draft length, and scroll offset. Ordinary candidate `3cd42fa098e0f7a4` then produced `4L/5R (prev 5) · full 2`, proving the ordinary route while isolating the remaining rebuilds to preview and lifecycle. The preview candidate removes `prev R`; explicit channel-enable/provider-change/plugin lifecycle `full` follows only after the preview field gate. Do not merge those slices or revive parent-forceUpdate, synchronous atomic rebuild, per-row history transactions, or long global debounce.
