...
The second resumed cut moves the translation cache's BDFDB persistence key, managed debounce timers, and source/signature/display/policy callback wiring into `translation-cache-wiring.js`. The cache store and every public runtime delegation remain unchanged; the legacy runtime keeps only its lazy singleton boundary and the ratchet drops from 3,448 to 3,431 lines. Deterministic build parity and all 1,282 tests pass; installed build `64ea246abd086f70` preserved all 301 live cache entries byte-for-byte across deployment, so this cut is ready for fast-forward merge.
