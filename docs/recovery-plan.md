...
The fifth resumed cut moves historical quiet-window managed timers, active-scroll state, queue identity, and snapshot-finish wiring into `historical-snapshot-cadence-wiring.js`. The 500 ms quiet window, active-scroll deferral, job-end sealing, waiting-batch merge, and live handoff policy remain unchanged; the legacy runtime keeps a compact lazy singleton and the ratchet drops from 3,404 to 3,401 lines. Deterministic build parity and all 1,294 tests pass; installed build `b99e4ae741747bed` left the complete configuration byte-identical. PTB confirmed one seal after continuous upward scrolling idled, no periodic capsule/repaint growth after completion, and one additional batch for the next newly loaded history segment; the cut is ready for fast-forward merge.
