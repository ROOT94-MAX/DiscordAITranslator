...
The third resumed cut moves provider-client request, timer, credential/language, notification, and AI prompt-policy wiring into `provider-client-wiring.js`. Managed retry timers still use BDFDB while backoff sleep remains deliberately raw, so plugin stop cannot strand an awaited promise. The provider client, engine catalog, wire adapters, and public runtime delegations remain unchanged; the legacy runtime keeps only its lazy singleton boundary and the ratchet drops from 3,431 to 3,415 lines. Deterministic build parity and all 1,286 tests pass; a real keyless English-to-Chinese request returned a valid translation without an error notification, and installed build `27a66a9c04c7b371` left the complete configuration byte-identical, so this cut is ready for fast-forward merge.
