...
插件停止前对全部频道执行同样恢复，再释放补丁和受管任务。消息编辑会捕获新原文签名并使旧请求/显示结果失效。频道会话裁剪只保留活跃请求、未确认恢复、手动抑制或原文档案仍需要的状态。
...
- 历史来源完整性、供应商物理中止和生命周期任务注册表仍在 `recovery-plan.md` 中开放；消息删除已改成直接订阅 Store dispatcher，目前等待 PTB 对真实删除事件形状的观察确认。
