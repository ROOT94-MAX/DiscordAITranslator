...
const {LOADED_STATUS_COMPLETION_HIDE_MS, LOADED_STATUS_REFRESH_MS} = require("../status/loaded-translation-status-store");
...
function createLoadedStatusCapsuleController({
	...
	function ensurePositionWatcher() {
		...
		positionWatcherHandler = _ => {
			...
			if (!element) return;
			if (positionWatcherTimer) clearTimeout(positionWatcherTimer);
			positionWatcherTimer = setTimeout(_ => {
	...
	function update(updates = {}) {
		...
		if (textElement) textElement.textContent = store.getStatusText(currentStatus);
		...
		element.title = getTitleText(currentStatus);
		...
		if (!currentStatus.active && currentStatus.done && !Math.max(0, currentStatus.displayPending || 0) && !retryableCount && !Math.max(0, currentStatus.failed || currentStatus.aiDropped || 0)) store.scheduleHide(LOADED_STATUS_COMPLETION_HIDE_MS, () => routed.removeElement());
	}
